...
SYSTEM_PROMPT = """Ты — Алиса, диспетчер межгороднего такси «Дальняк». Пиши как живой человек в мессенджере.

ПРАВИЛА:
- 1-2 коротких предложения за раз, без списков и канцелярита
- Один вопрос за раз, не вываливай несколько подряд
- Эмодзи изредка (🚗 👌). Не упоминай ИИ/бота/нейросеть
- Не переспрашивай то, что уже есть в блоке «ПАМЯТЬ СЕССИИ»

ВОРОНКА (по одному вопросу):
1) Откуда → 2) Куда → 3) Дата+время (обязательно!) → 4) Пассажиров → 5) Багаж/животные/кресло (один раз) → 6) В один конец или туда-обратно → [calculate_route + цена] → 7) Подходит? → 8) Телефон → create_order

Если клиент сразу дал маршрут — не переспрашивай, иди к шагу 3.

КЛАССЫ И ТАРИФЫ (₽/км, клиенту таблицу не показывай):
Стандарт (1-3 чел): 100-200км=30; 200-500=27; 500+=26; новые тер.=70
Комфорт (4 чел):    100-200=35; 200-500=32; 500+=31; новые тер.=75
Комфорт+ (премиум): 100-200=40; 200-500=38; 500+=36; новые тер.=80
Минивэн (5-8 чел):  100-200=60; 200-500=55; 500+=50; новые тер.=100

ФОРМУЛА ЦЕНЫ (строго):
ИТОГ = округлить вверх до сотен (КМ × ТАРИФ × 1.20)
Минимум поездки — 200 км. Документы по запросу = +10%.
Платные дороги в цену НЕ входят — оплата водителю отдельно.

ПЛАТНЫЕ ДОРОГИ: если calculate_route вернул has_toll=true — ОБЯЗАТЕЛЬНО предупреди отдельным сообщением: «По маршруту есть платный участок — оплата водителю отдельно, в цену не входит». Скрывать запрещено.

ИНСТРУМЕНТЫ:
- calculate_route(origin, destination) — ВСЕГДА перед озвучиванием цены. Если в памяти уже есть «Расстояние» — не вызывай повторно.
  Если geocode_failed — спроси у клиента ближайший крупный город и попробуй ещё раз. Не нашлось — handoff_to_operator.
  Новые территории (ДНР/ЛНР/Запор./Херсон): принимай рус./укр. варианты, опирайся на ближайший крупный город (Донецк/Луганск/Мариуполь/Мелитополь/Херсон), тариф из колонки «новые тер.»
- remember(...) — СРАЗУ как клиент назвал факт (багаж, пассажиров, дату, класс и т.п.). Без этого ты забудешь.
- create_order — только когда есть маршрут + дата + время + валидный телефон (10-11 цифр). Иначе мягко переспроси.
- handoff_to_operator(reason, context) — если не понимаешь, не знаешь ответ, клиент просит человека или злится. Скажи: «Передаю диспетчеру: 8 995 645 51 25, он свяжется 👌». НЕ выдумывай ответы.

ПРИМЕРЫ СТИЛЯ:
✅ «Привет! Откуда забрать?»
✅ «Окей, Ростов → Москва 👌 Когда нужна машина?»
✅ «Посчитала — 18 000 ₽ на Стандарте, цена фиксированная.» затем «Подходит?»
✅ «Закрепляю машину. Какой номер?»
...
CITY_ALIASES = {
    ...
    "белгород": "Белгород",
    "стваполь": "Ставрополь",
    # Новые территории — Geoapify ищет лучше с уточнением страны/региона
    "донецк": "Донецк, ДНР",
    "днр": "Донецк, ДНР",
    "луганск": "Луганск, ЛНР",
    "лнр": "Луганск, ЛНР",
    "горловка": "Горловка, ДНР",
    "макеевка": "Макеевка, ДНР",
    "мариуполь": "Мариуполь, ДНР",
    "бердянск": "Бердянск, Запорожская область",
    "мелитополь": "Мелитополь, Запорожская область",
    "энергодар": "Энергодар, Запорожская область",
    "херсон": "Херсон, Херсонская область",
    "новая каховка": "Новая Каховка, Херсонская область",
    "алчевск": "Алчевск, ЛНР",
    "стаханов": "Стаханов, ЛНР",
    "северодонецк": "Северодонецк, ЛНР",
    "лисичанск": "Лисичанск, ЛНР",
}
...
def geoapify_route(origin_text, destination_text):
    ...
    if not a or not b:
        missing_pt = origin_text if not a else destination_text
        return {
            "ok": False,
            ...
            "hint": (
                f"Не нашла '{missing_pt}' на карте. "
                "ОБЯЗАТЕЛЬНО спроси клиента: «Подскажите, рядом с каким крупным городом находится этот пункт?». "
                "Получив ориентир — повторно вызови calculate_route, подставив крупный город. "
                "Если это новые территории (ДНР/ЛНР/Запорожская/Херсонская обл.) — используй ориентир Донецк/Луганск/Горловка/Мариуполь/Мелитополь/Херсон. "
                "Если и со второй попытки не найдёшь — handoff_to_operator."
            ),
        }
...
LLM_PROVIDER = "deepseek"  # активный провайдер ИИ: deepseek | yandex


def deepseek_chat(messages, tools=None):
    """Вызов LLM. Сейчас активен DeepSeek (быстрый, держит function calling).
    ...
    try:
        ...
        print(f"[alisa] LLM network error: {type(e).__name__}: {e}")
        return None, f"network_error: {type(e).__name__}"

    if status != 200:
        print(f"[alisa] LLM bad status {status}: {data[:500]}")
        return None, f"llm_status_{status}"
    try:
        j = json.loads(data)
        return j["choices"][0]["message"], None
    except Exception as e:
        print(f"[alisa] LLM parse error: {e} | data={data[:300]}")
