...
const CITIES = [
  ...
  "Пенза","Астрахань","Липецк",
  "Тамбов","Тверь","Тула","Курск","Белгород","Брянск","Калуга",
  "Кострома","Владимир","Вологда","Смоленск","Орёл","Иваново",
  "Анапа","Сочи","Таганрог","Новороссийск","Ставрополь","Пятигорск",
  "Севастополь","Симферополь","Новые территории",
];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: NAVY, fontFamily: "Inter, sans-serif" }}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 flex flex-col" style={{ minHeight: "calc(100svh - 68px)", paddingTop: 20 }}>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:gap-16 flex-1">
            ...
            <div className="flex-1 md:max-w-xl">
              ...
              {/* статистика */}
              <div className="flex mt-7 mb-7 md:max-w-lg rounded-2xl overflow-hidden"
                style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
                {STATS.map((s, idx) => (
                  <div key={s.value} className="flex-1 flex flex-col items-center py-4 px-1"
                    style={{ borderLeft: idx === 0 ? "none" : "1px solid rgba(255,255,255,0.08)" }}>
                    <span style={{ fontFamily: "Oswald", fontWeight: 800, fontSize: "clamp(17px,2.1vw,24px)", color: GOLD2, lineHeight: 1, letterSpacing: "-0.01em" }}>{s.value}</span>
                    <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 9, textAlign: "center", marginTop: 4, lineHeight: 1.2, textTransform: "uppercase", letterSpacing: "0.04em" }}>{s.label}</span>
                  </div>
                ))}
              </div>
      ...
      {/* ══════════════════════════════════════
          СЕКЦИЯ: ГОРОДА
      ══════════════════════════════════════ */}
      <div style={{ background: `linear-gradient(180deg,${NAVY} 0%,#08080a 100%)` }}>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-6">
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: "Отзывы",      icon: "Star",       href: "/reviews", sub: `${REVIEWS.length} отзывов` },
            ].map(l => (
                  ...
                  <div className="flex gap-0.5 shrink-0 mt-0.5">
                    {[1,2,3,4,5].map(i => (
                      <Icon key={i} name="Star" size={11}
                        style={{ color: i <= (r.stars ?? 5) ? GOLD : "rgba(255,255,255,0.14)" }}
                        className={i <= (r.stars ?? 5) ? "fill-[#c9a84c]" : ""} />
                    ))}
                  </div>
                </div>
      ...
      ══════════════════════════════════════ */}
      <div className="hidden md:block" style={{ background: "#050505", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
